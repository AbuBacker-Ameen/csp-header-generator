...
def test_validate_csp_mismatching(
    csp_generator, tmp_path, mock_scanner, capsys, monkeypatch
):
    monkeypatch.setenv("CSP_PLAIN_OUTPUT", "1")  # Force plain text output
    csp_file = tmp_path / "csp.conf"
...
def test_validate_csp_non_existent_file(csp_generator, tmp_path):
    result = csp_generator.validate_csp(
        str(tmp_path / "nonexistent.conf"), str(tmp_path)
    )
    assert result is False
