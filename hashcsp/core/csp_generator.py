...
def is_tag(element: PageElement) -> TypeGuard[Tag]:
    """Type guard to check if a BeautifulSoup element is a Tag."""
    ...
    def scan_html_file(self, file_path: str):
        try:
            if not os.path.isfile(file_path):
                ...
                console.print(
                    f"[red]Error: {file_path} is not a valid file :no_entry_sign:[/red]"
                )
                return
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                for script in scripts:
                    if is_tag(script) and script.string:
                        script_hash = self.compute_hash(
                            script.string.strip(), file_path
                        )
                        if script_hash not in self.hashes["script-src"]:
                            self.hashes["script-src"].append(script_hash)
                            self.stats["unique_script_hashes"] += 1
                            logger.info(
                                f"Added script hash {script_hash} from {file_path}"
                            )
                    else:
                        logger.debug(f"Skipping non-Tag or empty script in {file_path}")

                for style in styles:
                    if is_tag(style) and style.string:
                        ...
                        if style_hash not in self.hashes["style-src"]:
                            ...
                            logger.info(
                                f"Added style hash {style_hash} from {file_path}"
                            )
                    else:
                        logger.debug(f"Skipping non-Tag or empty style in {file_path}")

                self.stats["files_processed"] += 1
        except FileNotFoundError:
            logger.error(f"File not found: {file_path}")
            console.print(
                f"[red]Error: File {file_path} not found :no_entry_sign:[/red]"
            )
        except UnicodeDecodeError:
            logger.error(f"Invalid encoding in file: {file_path}")
            console.print(
                f"[red]Error: File {file_path} has invalid encoding :no_entry_sign:[/red]"
            )
        except Exception as e:
            logger.error(f"Unexpected error scanning {file_path}: {e}", exc_info=True)
            console.print(
                f"[red]Unexpected error scanning {file_path}: {e} :cold_sweat:[/red]"
            )
    ...
    def scan_directory(self, path: str):
        try:
            if not os.path.isdir(path):
                ...
                console.print(
                    f"[red]Error: {path} is not a valid directory :no_entry_sign:[/red]"
                )
                return
            ...
            if html_files == 0:
                ...
                console.print(
                    f"[yellow]No HTML files found in {path} :open_file_folder:[/yellow]"
                )
            else:
            ...
            console.print(
                f"[red]Error scanning directory {path}: {e} :cold_sweat:[/red]"
            )
    ...
    def fetch_remote_site(self, url: str) -> bool:
        try:
            ...
            if not scripts and not styles:
                ...
                console.print(
                    f"[yellow]No inline scripts or styles at {url} :satellite:[/yellow]"
                )
            ...
            for script in scripts:
                if is_tag(script) and script.string:
                    ...
                    logger.debug(f"Skipping non-Tag or empty script at {url}")
            ...
            for style in styles:
                if is_tag(style) and style.string:
                    ...
                    logger.debug(f"Skipping non-Tag or empty style at {url}")
            ...
            for script in soup.find_all("script", src=True):
                if is_tag(script):
                    ...
                    logger.debug(f"Skipping non-Tag script with src at {url}")
            ...
            for img in soup.find_all("img", src=True):
                if is_tag(img):
                    ...
                    logger.debug(f"Skipping non-Tag img at {url}")
            ...
            console.print(
                f"[red]HTTP error: {e.response.status_code} {e.response.reason} :no_entry_sign:[/red]"
            )
            return False
        except ConnectionError:
            logger.error(f"Connection error fetching {url}: Unable to reach server")
            console.print(
                f"[red]Connection error: Unable to reach {url}. Check your network. :globe_with_meridians:[/red]"
            )
            return False
        except Timeout:
            logger.error(f"Timeout fetching {url}: Request took too long")
            console.print(
                f"[red]Timeout: Request to {url} took too long. Try again later. :hourglass_flowing_sand:[/red]"
            )
            return False
            ...
            console.print(
                f"[red]Unexpected error processing {url}: {e} :cold_sweat:[/red]"
            )
            return False
    ...
    def generate_csp(self, report: bool = True) -> str:
        try:
            ...
            if not csp_parts:
                ...
                console.print(
                    "[yellow]No directives to generate CSP header :scroll:[/yellow]"
                )
    ...
    def validate_csp(self, csp_file: str, path: str) -> bool:
        ...
        try:
            if not os.path.isfile(csp_file):
                ...
                console.print(
                    f"[red]Error: CSP file {csp_file} not found :no_entry_sign:[/red]"
                )
                return False
            ...
            console.print(
                f"[red]Error: CSP file {csp_file} has invalid encoding :no_entry_sign:[/red]"
            )
            return False
        except Exception as e:
            logger.error(f"Error validating CSP file {csp_file}: {e}", exc_info=True)
            console.print(
                f"[red]Error validating CSP file {csp_file}: {e} :cold_sweat:[/red]"
            )
            return False
    ...
    def update_directive(self, directive: str, sources: List[str]):
        try:
            if directive in self.directives:
                ...
                console.print(
                    f"[green]Updated {directive} with sources: {sources} :white_check_mark:[/green]"
                )
            else:
                logger.error(f"Invalid directive: {directive}")
                console.print(
                    f"[red]Error: Invalid directive: {directive}. Valid directives: {list(self.directives.keys())} :no_entry_sign:[/red]"
                )
                raise ValueError(f"Invalid directive: {directive}")
        except Exception as e:
            logger.error(f"Error updating directive {directive}: {e}", exc_info=True)
            console.print(
                f"[red]Error updating directive {directive}: {e} :cold_sweat:[/red]"
            )
            raise
